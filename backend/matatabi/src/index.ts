...
import { VerifyFirebaseAuthEnv, getFirebaseToken, verifyFirebaseAuth } from '@hono/firebase-auth';
...
app
	...
	.post('/login/emailPass', async (c) => {
		...
		setCookie(c, 'session', sessionCookie, { maxAge: expiresIn, sameSite: 'None', secure: true });
		return c.json({ token, user });
	})
